...
/**
 *  source .env && forge script script/ArchitectureDeployments/Avalanche/rggAVAX/DeployRggavaxDefaultCollateralUManager.s.sol:DeployRggavaxDefaultCollateralUManager --slow --broadcast --verifier-url 'https://api.routescan.io/v2/network/mainnet/evm/43114/etherscan' --etherscan-api-key "verifyContract" --verify
 */

contract DeployRggavaxDefaultCollateralUManager is MerkleTreeHelper, ContractNames, AvalancheAddresses, Roles {
    ...
    function generateSniperMerkleRoot() public {
        ...
        _addSuzakuApproveAndDepositLeaf(leafs, getAddress(sourceChain, "DC_ggAVAX"));

        string memory filePath = "./leafs/avalanche_rggAVAX_suzaku_dc_manager_internal.json";
        ...
        suzakuUManager.setConfiguration(
            DefaultCollateral(getAddress(sourceChain, "DC_ggAVAX")), MIN_DEPOSIT, rawDataDecoderAndSanitizer
        );
        ...
        console.log(
            "This script generated './leafs/avalanche_rggAVAX_suzaku_dc_manager_internal.json', but did not set the manage root for the deployed uManager!"
        );
